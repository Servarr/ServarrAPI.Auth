...
using ServarrAuthAPI.Services.Trakt.Responses;

namespace ServarrAuthAPI.Services.Spotify
{
    public class SpotifyService
    {
        private const string BaseUrl = "https://accounts.spotify.com/";

        private readonly TraktOptions _options;
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public SpotifyService(IOptions<TraktOptions> traktOptions, IHttpClientFactory httpClientFactory, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<string> CreateAuthorizationUrlAsync(string state)
        {
            ...
            return $"{BaseUrl}/authorize?{encodedUri}";
        }

        public async Task<OAuthTokenResponse> GetAuthorizationAsync(string code)
        {
            var client = _httpClientFactory.CreateClient(nameof(SpotifyService));
        ...
        private string GetRedirectUri()
        {
            ...
            redirectUri.Append("/v1/spotify/callback");
